...
The release workflow also publishes a signed Nix binary cache on GitHub Pages,
so Nix downloads the prebuilt closure instead of compiling. Configure the
substituter and the cache's trust key once:
...
After that, `nix profile install github:grigio/jcode` (and `nixos-rebuild`)
download the prebuilt jcode closure from the cache instead of building it.
Paths the cache does not have fall back to building from source.

> **Repo setup (one-time):** the cache is generated by the same CI that
> publishes the release, but it is skipped until two things are configured:
